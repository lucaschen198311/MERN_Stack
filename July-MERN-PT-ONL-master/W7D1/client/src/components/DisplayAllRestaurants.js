      ...
      {restaurants.length > 0 &&
        restaurants.map((restaurant, index) => (
          <div key={index}>
            ...
            <h2>DISHES</h2>
            {/* restaurant.dishes is an array of strings, so we need to loop through this array to get all the value */}
            {/* first, check if restaurant.dishes exists */}
            {restaurant.dishes &&
              restaurant.dishes.map((dish, index) => <p key={index}>{dish}</p>)}
